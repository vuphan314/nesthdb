...
def var2col_minors(node,var,minors):
    if var in minors:
        return "{}.val".format(var2tab_alias(node, var))
    else:
        return f"v{var}"

def lit2var_minors (node,lit,minors):
    return var2col_minors(node,abs(lit),minors)

def lit2expr_minors (node,lit,minors):
    if lit > 0:
        return lit2var_minors(node,lit,minors)
    else:
        return "NOT {}".format(lit2var_minors(node,lit,minors))
...
class NestPmc(Problem):
    ...
    def filter(self,node):
        ...
        if len(node.minor_vertices) > 0 and len(node.all_vertices) - len(node.vertices) <= self.inner_vars_threshold:
            ...
            f += "({0})".format(") AND (".join(
                [" OR ".join([lit2expr_minors(node,c,minor_vertices) for c in clause]) for clause in cur_cl]
            ))
            f += ")"
        return f
