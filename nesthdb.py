...
class Problem:
    ...
    def choose_subset(self):
        ...
        for enc in cfg_asp["encodings"]:
            if interrupted:
                return
            if not self.kwargs["no_abstraction_cache"]:
                if self.non_nested in abstraction_cache and enc["file"] in abstraction_cache[self.non_nested]:
                    self.non_nested = abstraction_cache[self.non_nested][enc["file"]]
                    logger.info("Abstraction cache hit")
                    continue
            size = enc["size"]
            ...
            if len(res) == 0:
                ...
                new_result = frozenset(res[0])
                if not self.kwargs["no_abstraction_cache"]:
                    if self.non_nested in abstraction_cache:
                        abstraction_cache[self.non_nested].update({enc["file"]: new_result})
                    else:
                        abstraction_cache[self.non_nested] = {enc["file"]: new_result}
                self.non_nested = new_result
    ...
    def solve(self):
        ...
        self.non_nested = frozenset(self.non_nested.intersection(self.projected))
...
def main():
    ...
    arg_parser.add_argument("--sharpsat", dest="sharpsat", help="Calculate model count instead of projected model count", action="store_true")
    arg_parser.add_argument("--no-cache", dest="no_cache", help="Disable solver cache", action="store_true")
    arg_parser.add_argument("--no-abstraction-cache", dest="no_abstraction_cache", help="Disable abstraction cache", action="store_true")
    args = parse_args(arg_parser)
    cfg = read_cfg(args.config)
    fname = args.file

    if args.sharpsat:
        formula = Formula.sharpsat_from_file(fname)
    else:
